...
const bool    skip_startup    = true;                // Skip intro splash screen for testing
...
// Also can use integers instead of bool. 0 does nothing.
void next_screen(int upOrDown = 1){
  if (!upOrDown) return;
  bool dir = (upOrDown > 0) ? true: false;
  next_screen(dir);
}
